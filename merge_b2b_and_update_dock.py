...
# 1. Canonical Dock with merged "Dystrybucja & B2B" (truck removed, clean handshake/network icon kept)
canonical_dock = """<nav class="prescot-dock" aria-label="Nawigacja główna">
  ...
  <a href="/dystrybucja/" class="dock-item" data-tooltip="Dystrybucja & B2B" aria-label="Dystrybucja & Strefa B2B">
...
# 2. Synchronize dock across all HTML files
        ...
        if f.endswith(".html"):
            ...
            if '<nav class="prescot-dock"' in content:
                ...
                with open(fpath, "w", encoding="utf-8") as file:
                    file.write(content)
                print(f"Updated merged dock in {fpath}")

# 3. Update active indicator matcher in local-navigation.js to recognize both /dystrybucja/ and /wspolpraca-b2b/
nav_js_path = os.path.join(base_dir, "local-navigation.js")
with open(nav_js_path, "r", encoding="utf-8") as f:
    nav_js = f.read()

nav_active_patch = """  // 1. Podświetlanie aktywnej pozycji w menu dock
  ...
  document.querySelectorAll(".prescot-dock .dock-item").forEach((item) => {
    ...
    if (normalizedHref === currentPath || 
       (normalizedHref === '/dystrybucja/' && currentPath === '/wspolpraca-b2b/') ||
       (normalizedHref === '/wspolpraca-b2b/' && currentPath === '/dystrybucja/')) {
      item.classList.add("url-active");
    }
  });"""

nav_js = re.sub(
    r'// 1\. Podświetlanie aktywnej pozycji w menu dock.*?\}\);',
    nav_active_patch,
    nav_js,
    flags=re.DOTALL
)
...
with open(nav_js_path, "w", encoding="utf-8") as f:
    f.write(nav_js)
print("Updated local-navigation.js active URL matching for merged B2B & Dystrybucja.")

# 4. Synchronize unified content between dystrybucja/index.html and wspolpraca-b2b/index.html
dystr_path = os.path.join(base_dir, "dystrybucja/index.html")
b2b_path = os.path.join(base_dir, "wspolpraca-b2b/index.html")

with open(dystr_path, "r", encoding="utf-8") as f:
    dystr_content = f.read()

# Ensure breadcrumbs and title are unified
dystr_content = dystr_content.replace(
    "<title>Dystrybucja B2B & Współpraca Handlowa — Prescot LED</title>",
    "<title>Dystrybucja & Strefa Partnera B2B — Prescot LED</title>"
)

# Write to both dystrybucja and wspolpraca-b2b
with open(dystr_path, "w", encoding="utf-8") as f:
    f.write(dystr_content)
with open(b2b_path, "w", encoding="utf-8") as f:
    f.write(dystr_content)

print("Unified Dystrybucja & Strefa B2B across both endpoints.")
