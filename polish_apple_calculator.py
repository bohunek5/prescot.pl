...
# Polished Apple Configurator CSS
polished_calc_css = """
  /* =========================================================
     APPLE LUXURY ARCHITECTURAL CONFIGURATOR — POLISHED
     ========================================================= */
  .apple-calc-wrapper {
    width: 100%;
    max-width: 1240px;
    ...
    border-radius: 26px;
    box-shadow: 0 24px 60px -15px rgba(15, 23, 42, 0.07), 0 2px 6px rgba(0,0,0,0.02);
    overflow: hidden;
    color: #1e293b;
  }
  ...
  @media (max-width: 800px) {
    .apple-calc-hero { padding: 30px 24px; flex-direction: column; align-items: flex-start; }
  }
  ...
  @media (max-width: 800px) {
    .apple-calc-config { padding: 30px 24px; }
  }
  ...
  /* Segmented Controls (Clean 12V / 24V / 48V) */
  .apple-segmented {
    display: grid;
  ...
  .apple-seg-btn.active {
    background: #ffffff;
    color: #0f172a;
    box-shadow: 0 3px 12px rgba(0,0,0,0.07), 0 1px 3px rgba(0,0,0,0.03);
  }
  ...
  /* Chip grid for clean power and wire tags */
  .apple-chip-grid {
    display: grid;
  ...
  .apple-range-slider::-webkit-slider-thumb {
    ...
    border-radius: 50%;
    background: #ffffff;
    border: 2.5px solid #e55933;
    box-shadow: 0 2px 8px rgba(229, 89, 51, 0.35);
    cursor: pointer;
    transition: transform 0.15s ease, box-shadow 0.15s ease;
  }
  .apple-range-slider::-webkit-slider-thumb:hover {
    transform: scale(1.18);
    box-shadow: 0 3px 12px rgba(229, 89, 51, 0.5);
  }
  .apple-num-input {
    width: 86px;
    padding: 10px 12px;
  ...
  @media (max-width: 960px) {
    .apple-calc-results { border-left: none; border-top: 1px solid #f1f5f9; padding: 34px 24px; }
  }
  ...
  .metric-card.hero-metric {
    ...
    box-shadow: 0 4px 16px rgba(229, 89, 51, 0.08);
  }
  .metric-label {
    font-size: 12px;
    color: #64748b;
...
# Read Baza Wiedzy template and inject
...
<head>
  ...
  <link rel="stylesheet" href="/prescot-global.css?v=20260901-apple-calc-v3">
...
<main id="content-start" class="prescot-main-container">
  ...
  <!-- POLISHED APPLE-GRADE CALCULATOR -->
...
<script src="/local-navigation.js?v=20260901-apple-calc-v3" defer></script>
</body>
</html>
""")

print("Clean, refined Apple Configurator successfully written to Baza Wiedzy.")
