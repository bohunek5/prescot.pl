...
template_path = os.path.join(base_dir, "wspolpraca-b2b/index.html")

with open(template_path, 'r', encoding='utf-8') as f:
    template_html = f.read()

# 1. Extract head from template
head_match = re.search(r'(.*?<head>.*?</head>)', template_html, re.DOTALL | re.IGNORECASE)
if not head_match:
    print("Could not find head in template")
    exit(1)
template_head = head_match.group(1)

# 2. Extract header body start to the content container
header_match = re.search(r'</head>\s*(<body.*?>.*?<div id="content"[^>]*>\s*<div class="ast-container">)', template_html, re.DOTALL | re.IGNORECASE)
if not header_match:
    print("Could not find body/header in template")
    exit(1)
template_body_start = header_match.group(1)

# 3. Extract footer to end of file
footer_match = re.search(r'(</div>\s*<!-- \.ast-container -->.*?</html>)', template_html, re.DOTALL | re.IGNORECASE)
if not footer_match:
    # fallback to just </div><!-- #content -->
    footer_match = re.search(r'(</div>\s*<!-- #content -->.*?</html>)', template_html, re.DOTALL | re.IGNORECASE)

if footer_match:
    template_footer = footer_match.group(1)
else:
    # let's try just the footer tag
    footer_match = re.search(r'(<footer data-elementor-type="footer".*?</html>)', template_html, re.DOTALL | re.IGNORECASE)
    template_footer = "\t\t</div><!-- ast-container -->\n\t</div><!-- content -->\n" + footer_match.group(1)

dock_snippet = """<!-- GLOBAL MENU START -->
...
<!-- GLOBAL TRANSLATE END -->
<script src="/local-navigation.js?v=20260901-white-dock" defer></script>
"""

# Replace any existing prescot-dock in the footer with our snippet just in case
template_footer = re.sub(r'<!-- GLOBAL MENU START -->.*<script src="/local-navigation.js\?v=20260901-white-dock" defer></script>', '', template_footer, flags=re.DOTALL)
# And make sure it's inserted before </body>
template_footer = template_footer.replace('</body>', dock_snippet + '\n</body>')


def wrap_page(page_name, extra_css):
    path = os.path.join(base_dir, page_name)
    with open(path, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Extract inner content
    start = content.find('<main id="main">')
    if start == -1:
        start = content.find('<div class="page-glow')
    if start == -1:
        print(f"Error: could not find start of content in {page_name}")
        return
        
    end = content.find('<nav class="prescot-dock"')
    if end == -1:
    ...
    # Combine
    # Also inject the extra_css before </head>
    custom_head = template_head.replace('</head>', f'<link rel="stylesheet" href="/{extra_css}">\n<link rel="stylesheet" href="/prescot-global.css?v=20260901-white-dock">\n</head>')
    
    final_html = custom_head + "\n" + template_body_start + "\n" + inner_content + "\n" + template_footer
    
    with open(path, 'w', encoding='utf-8') as f:
        f.write(final_html)
    print(f"Wrapped {page_name}")

wrap_page("kontakt/index.html", "contact.css")
wrap_page("baza-wiedzy/index.html", "baza-wiedzy.css?v=20260901-responsive-cards")
