...
# Re-apply across all html files
...
for file_path in html_files:
    ...
    # Check if file has a dock
    dock_match = re.search(r'<!-- GLOBAL MENU START -->.*?</html>|<nav class="prescot-dock".*?</html>', content, re.DOTALL)
    if dock_match:
        content = content[:dock_match.start()] + unified_dock
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(content)
        count += 1

print(f"Updated unified dock in {count} HTML files!")
