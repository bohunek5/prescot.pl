# -*- coding: utf-8 -*-
import os
import re

base_dir = "/Users/karolbohdanowicz/safe_backup/tasmaled-local/public"

# 1. Update prescot-global.css with polished Apple-grade dialog styles and animations
css_path = os.path.join(base_dir, "prescot-global.css")
with open(css_path, "r", encoding="utf-8") as f:
    css = f.read()

b2c_dialog_css = """
/* =========================================================
   NATIVE B2C REDIRECT POPUP DIALOG (APPLE-GRADE & HUMAN)
   ========================================================= */
.prescot-b2c-dialog {
  border: none !important;
  background: transparent !important;
  padding: 0 !important;
  max-width: 440px !important;
  width: calc(100% - 32px) !important;
  margin: auto !important;
  border-radius: 26px !important;
  box-shadow: 0 32px 80px -12px rgba(15, 23, 42, 0.5) !important;
  outline: none !important;
  overflow: visible !important;
}

.prescot-b2c-dialog::backdrop {
  background: rgba(15, 23, 42, 0.72) !important;
  backdrop-filter: blur(10px) !important;
  -webkit-backdrop-filter: blur(10px) !important;
  animation: pFadeIn 0.25s ease-out !important;
}

.b2c-dialog-box {
  background: #ffffff !important;
  border-radius: 26px !important;
  padding: 34px 28px 28px 28px !important;
  text-align: center !important;
  position: relative !important;
  border: 1px solid #e2e8f0 !important;
  box-sizing: border-box !important;
  font-family: 'Manrope', -apple-system, BlinkMacSystemFont, sans-serif !important;
  animation: pPopIn 0.28s cubic-bezier(0.16, 1, 0.3, 1) !important;
}

@keyframes pPopIn {
  0% { transform: scale(0.92); opacity: 0; }
  100% { transform: scale(1); opacity: 1; }
}
@keyframes pFadeIn {
  0% { opacity: 0; }
  100% { opacity: 1; }
}

.b2c-dialog-icon-wrap {
  position: relative;
  width: 64px;
  height: 64px;
  margin: 0 auto 16px auto;
}

.b2c-dialog-icon {
  width: 64px !important;
  height: 64px !important;
  border-radius: 20px !important;
  background: linear-gradient(135deg, #ff7a50 0%, #e55933 100%) !important;
  color: #ffffff !important;
  display: flex !important;
  align-items: center !important;
  justify-content: center !important;
  box-shadow: 0 10px 24px rgba(229, 89, 51, 0.35) !important;
}

.b2c-dialog-badge {
  display: inline-flex !important;
  align-items: center !important;
  gap: 6px !important;
  font-size: 11.5px !important;
  font-weight: 800 !important;
  text-transform: uppercase !important;
  letter-spacing: 0.8px !important;
  color: #e55933 !important;
  background: rgba(229, 89, 51, 0.08) !important;
  border: 1px solid rgba(229, 89, 51, 0.2) !important;
  padding: 5px 14px !important;
  border-radius: 999px !important;
  margin-bottom: 12px !important;
}

.b2c-dialog-box h3 {
  font-family: 'Outfit', sans-serif !important;
  font-size: 23px !important;
  font-weight: 800 !important;
  color: #0f172a !important;
  margin-bottom: 8px !important;
  letter-spacing: -0.02em !important;
  line-height: 1.2 !important;
}

.b2c-dialog-box p.b2c-main-desc {
  font-size: 14px !important;
  color: #475569 !important;
  line-height: 1.55 !important;
  margin-bottom: 18px !important;
}

.b2c-feature-pills {
  display: flex !important;
  justify-content: center !important;
  gap: 8px !important;
  flex-wrap: wrap !important;
  margin-bottom: 24px !important;
}

.b2c-pill {
  font-size: 11.5px !important;
  font-weight: 700 !important;
  color: #1e293b !important;
  background: #f1f5f9 !important;
  padding: 5px 11px !important;
  border-radius: 8px !important;
  display: inline-flex !important;
  align-items: center !important;
  gap: 4px !important;
}

.b2c-dialog-actions {
  display: flex !important;
  flex-direction: column !important;
  gap: 10px !important;
}

.b2c-btn-confirm {
  background: linear-gradient(135deg, #e55933 0%, #d04620 100%) !important;
  color: #ffffff !important;
  padding: 14px 22px !important;
  border-radius: 14px !important;
  font-weight: 800 !important;
  font-size: 15px !important;
  text-decoration: none !important;
  display: flex !important;
  align-items: center !important;
  justify-content: center !important;
  gap: 8px !important;
  transition: all 0.2s cubic-bezier(0.16, 1, 0.3, 1) !important;
  box-shadow: 0 6px 18px rgba(229, 89, 51, 0.32) !important;
}

.b2c-btn-confirm:hover {
  background: linear-gradient(135deg, #f0643d 0%, #c9401b 100%) !important;
  transform: translateY(-2px) !important;
  box-shadow: 0 8px 24px rgba(229, 89, 51, 0.42) !important;
}

.b2c-btn-cancel {
  background: transparent !important;
  color: #64748b !important;
  border: 1px solid #e2e8f0 !important;
  padding: 12px 20px !important;
  border-radius: 14px !important;
  font-weight: 700 !important;
  font-size: 13.5px !important;
  cursor: pointer !important;
  transition: all 0.2s ease !important;
}

.b2c-btn-cancel:hover {
  background: #f8fafc !important;
  color: #0f172a !important;
  border-color: #cbd5e1 !important;
}

.b2c-dialog-close {
  position: absolute !important;
  top: 14px !important;
  right: 16px !important;
  width: 32px !important;
  height: 32px !important;
  border-radius: 50% !important;
  border: none !important;
  background: #f1f5f9 !important;
  color: #64748b !important;
  font-size: 20px !important;
  line-height: 1 !important;
  cursor: pointer !important;
  display: flex !important;
  align-items: center !important;
  justify-content: center !important;
  transition: all 0.2s !important;
}

.b2c-dialog-close:hover {
  background: #e2e8f0 !important;
  color: #0f172a !important;
}
"""

if "NATIVE B2C REDIRECT POPUP DIALOG" in css:
    css = re.sub(r'/\* =+ \s*NATIVE B2C REDIRECT POPUP DIALOG.*?(?=\n/\*|\Z)', b2c_dialog_css, css, flags=re.DOTALL)
else:
    css += "\n" + b2c_dialog_css

with open(css_path, "w", encoding="utf-8") as f:
    f.write(css)
print("Updated prescot-global.css with Apple-grade human dialog styles.")

# 2. Update local-navigation.js with human copy and clean UI elements
nav_js_path = os.path.join(base_dir, "local-navigation.js")
with open(nav_js_path, "r", encoding="utf-8") as f:
    nav_js = f.read()

# Replace the dialog HTML in local-navigation.js
clean_dialog_html = """        <div class="b2c-dialog-box">
          <button type="button" class="b2c-dialog-close" id="b2cCloseCross" aria-label="Zamknij">&times;</button>
          
          <div class="b2c-dialog-icon-wrap">
            <div class="b2c-dialog-icon">
              <svg viewBox="0 0 576 512" width="28" height="28" xmlns="http://www.w3.org/2000/svg">
                <path fill="currentColor" d="M576 216v16c0 13.255-10.745 24-24 24h-8l-26.113 182.788C514.509 462.435 494.257 480 470.37 480H105.63c-23.887 0-44.139-17.565-47.518-41.212L32 256h-8c-13.255 0-24-10.745-24-24v-16c0-13.255 10.745-24 24-24h67.341l106.78-146.821c10.395-14.292 30.407-17.453 44.701-7.058 14.293 10.395 17.453 30.408 7.058 44.701L170.477 192h235.046L326.12 82.821c-10.395-14.292-7.234-34.306 7.059-44.701 14.291-10.395 34.306-7.235 44.701 7.058L484.659 192H552c13.255 0 24 10.745 24 24zM312 392V280c0-13.255-10.745-24-24-24s-24 10.745-24 24v112c0 13.255 10.745 24 24 24s24-10.745 24-24zm112 0V280c0-13.255-10.745-24-24-24s-24 10.745-24 24v112c0 13.255 10.745 24 24 24s24-10.745 24-24zm-224 0V280c0-13.255-10.745-24-24-24s-24 10.745-24 24v112c0 13.255 10.745 24 24 24s24-10.745 24-24z"/>
              </svg>
            </div>
          </div>

          <div class="b2c-dialog-badge">Sklep Internetowy</div>
          <h3>Oficjalny Sklep Prescot</h3>
          <p class="b2c-main-desc">Zakupy detaliczne, taśmy LED na metry, zasilacze i profile aluminiowe kupisz bezpośrednio w naszym sklepie internetowym.</p>
          
          <div class="b2c-feature-pills">
            <span class="b2c-pill">⚡ Wysyłka 24h</span>
            <span class="b2c-pill">🔒 Zakupy online</span>
            <span class="b2c-pill">📦 Ponad 500+ produktów</span>
          </div>

          <div class="b2c-dialog-actions">
            <a href="https://prescot.com.pl/" id="b2cConfirmBtn" target="_blank" rel="noopener" class="b2c-btn-confirm">
              Przejdź do sklepu prescot.com.pl &rarr;
            </a>
            <button type="button" class="b2c-btn-cancel" id="b2cCancelBtn">
              Zostań na tej stronie
            </button>
          </div>
        </div>"""

nav_js = re.sub(
    r'<div class="b2c-dialog-box">.*?</div>\s*`;',
    clean_dialog_html + '\n      `;',
    nav_js,
    flags=re.DOTALL
)

with open(nav_js_path, "w", encoding="utf-8") as f:
    f.write(nav_js)
print("Updated local-navigation.js with human, polished copy and features.")
