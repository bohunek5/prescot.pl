...
new_dock_html = """<!-- GLOBAL MENU START -->
<nav class="prescot-dock" aria-label="Nawigacja główna">
  ...
  <!-- 1. Strefa B2B (ikona dystrybucji) -->
  ...
  <!-- 3. Sklep B2C (ikona sklepu storefront) -->
...
for file_path in html_files:
    ...
    # Regex to find <nav class="prescot-dock"...</nav>
    dock_pattern = re.compile(r'<!-- GLOBAL MENU START -->.*?<!-- GLOBAL MENU END -->|<nav class="prescot-dock".*?</nav>', re.DOTALL)
    if dock_pattern.search(content):
        content = dock_pattern.sub(new_dock_html, content)
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(content)
        count += 1

print(f"Successfully replaced dock in {count} HTML files!")

# Also re-run update_b2b_pages.py and generate_perfect_pages.py to sync everything
os.system(f"python3 {os.path.join(base_dir, 'update_b2b_pages.py')}")
os.system(f"python3 {os.path.join(base_dir, 'build_clean_standalone_pages.py')}")
print("All pages fully synchronized with new dock structure!")
