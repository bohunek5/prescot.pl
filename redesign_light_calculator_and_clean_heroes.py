...
# Baza Wiedzy Page HTML
baza_html = f"""<!DOCTYPE html>
<html lang="pl">
<head>
  <meta charset="UTF-8">
  ...
  <link rel="stylesheet" href="/prescot-global.css?v=20260902-baza-clean-v8">
  ...
  <!-- SCHEMA.ORG FAQPAGE & BREADCRUMBS -->
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  /* FULL-SCREEN 100VH HERO SLIDE (BEZ DYMKÓW) */
  .p-full-hero {{
    position: relative;
    width: 100%;
    min-height: 100vh;
  ...
  /* =========================================================
     APPLE-GRADE LIGHT & CLEAN CALCULATOR (BEZ CIEMNYCH GRANATÓW)
     ========================================================= */
  .apple-calc-card {{
    background: #ffffff;
    border: 1px solid var(--p-border);
  ...
  /* HERO METRIC: CZYSTY, JASNY, WYRÓŻNIONY POMARAŃCZĄ */
  .metric-card.hero-metric {{
    grid-column: 1 / -1;
    background: #ffffff;
    color: var(--p-dark);
    border: 2px solid var(--p-primary);
    box-shadow: 0 8px 24px rgba(229, 89, 51, 0.08);
  }}
  .metric-label {{
    font-size: 12px;
    color: var(--p-text-muted);
  ...
  /* REKOMENDACJA: CZYSTY JASNY KARTON Z DELIKATNYM AKCENTEM */
  .calc-rec-card {{
    background: linear-gradient(135deg, #fffaf8 0%, #ffffff 100%);
    border-radius: 16px;
    padding: 24px;
    color: var(--p-dark);
    border: 1px solid rgba(229, 89, 51, 0.28);
    box-shadow: 0 6px 20px rgba(229, 89, 51, 0.06);
    display: flex;
    flex-direction: column;
    gap: 10px;
  }}
...
<!-- FULL-SCREEN 100VH HERO SLIDE (CZYSTE H1 I LEAD BEZ DYMKÓW) -->
...
<main id="content-start" class="prescot-main-container">
  ...
  <!-- 1. BAZA WIEDZY / BLOG Z 16 ARTYKUŁAMI -->
  ...
  <!-- 2. APPLE-GRADE LIGHT CALCULATOR SECTION (PONIŻEJ BLOGA) -->
  <section id="kalkulator-led" class="apple-calc-card">
    ...
    <div class="apple-calc-body">
      ...
      <div class="apple-calc-results">
        ...
        <!-- Recommendation Box (Clean Light Luxury) -->
  ...
  <section id="faq-section" style="margin-bottom: 70px;">
    <div style="text-align:center; max-width:720px; margin:0 auto 36px auto;">
      <h2 style="font-family:'Outfit',sans-serif; font-size: clamp(26px, 3vw, 34px); font-weight: 700; color: var(--p-dark); margin-bottom: 10px; letter-spacing: -0.02em;">Najczęściej zadawane pytania (FAQ)</h2>
...
<script src="/local-navigation.js?v=20260902-baza-clean-v8" defer></script>
</body>
</html>
"""
...
with open(baza_path, "w", encoding="utf-8") as f:
    f.write(baza_html)
print("Updated baza-wiedzy/index.html with light calculator and clean hero.")

# Clean Hero Eyebrows from Dystrybucja, Kontakt, and Wspolpraca-B2B
for fname in ["dystrybucja/index.html", "kontakt/index.html", "wspolpraca-b2b/index.html"]:
    fpath = os.path.join(base_dir, fname)
    with open(fpath, "r", encoding="utf-8") as file:
        content = file.read()
    
    # Remove <div class="p-hero-eyebrow">...</div>
    content = re.sub(r'<div class="p-hero-eyebrow">.*?</div>', '', content, flags=re.DOTALL)
    
    with open(fpath, "w", encoding="utf-8") as file:
        file.write(content)
    print(f"Removed eyebrow bubble from {fname}")

print("All heroes cleaned and typography structure normalized.")
